...
主题：**收尾与一致性**。对齐 v1.4/v1.5 后遗留的英文版与文档，并确认 HF Demo 自定义主题输入在线。

### 新增
- 📗 **英文版 SKILL.en.md 同步到 v1.5**：frontmatter 版本/触发词（kps）、fail-loud 渲染流程（读 stderr→修 JSON、--strict 退出码 0/2/3、禁止 ASCII 降级）、新增 Step 9 quiz 分支、输入契约与示例补 kps、异常表与自查清单对齐。
- 🌐 **HF Demo 自定义主题输入**：访客可在线输入任意主题并选阵型，立即生成带主题的阵型框架（主仓与 HF Space 已同步部署）。

### 变更
- 📋 README Roadmap 更新到真实进度（v1.0–v1.5 全勾，列出 v1.6–v2.0 路线）。
