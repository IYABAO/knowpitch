...
def draw_team(svg, box, formation, coach, players, bench, flow, topic_label,
    ...
    svg.append(f'<text x="{(x0+x1)/2:.0f}" y="{y0-70}" text-anchor="middle" '
               f'font-size="36" font-weight="bold" fill="{TEXT_COLOR}">{title}</text>')
...
def main():
    ...
    svg = []
    svg.append(f'<svg xmlns="http://www.w3.org/2000/svg" width="2000" height="{base_height + bench_extra}" '
               f'viewBox="0 0 2000 {base_height + bench_extra}" '
               f'font-family="\'PingFang SC\',\'Microsoft YaHei\','
               f'\'Noto Sans CJK SC\',sans-serif">')
    svg.append(f'<rect width="2000" height="{base_height + bench_extra}" fill="#F4F7F2"/>')

    if b_team:
        main_box = (60, 140, 940, 1300)
        b_box = (1060, 140, 1940, 1300)
