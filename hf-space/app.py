...
def format_player_cards(team_data):
    """格式化球员卡文本"""
    lines = []
...
def on_select_example(topic):
    ...
    player_cards = format_player_cards(team_data)
    tactic_board = format_tactic_board(team_data)

    return svg_path, player_cards, tactic_board, f"已加载：{topic}"
    ...
    **触发词**：kp11 / kpt / kpitch / knowpitch
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            load_btn = gr.Button("📊 生成阵型图", variant="primary")
